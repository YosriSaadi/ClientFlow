...
class HistDialog(QDialog):
    def __init__(self, history, vente_id=None, parent=None):
        ...
        for row, record in enumerate(history):
            date_paiement, montant, mode = record[:3]  # Only use first 3 columns
            self.table.insertRow(row)

            self.table.setItem(row, 0, QTableWidgetItem(str(date_paiement)))
            self.table.setItem(row, 1, QTableWidgetItem(f"{montant:.3f}"))
    ...
    def print_history(self):
        ...
        if result:
            ...
            client_info = f"Client: {client_nom} | Description: {description} | Montant Total: {montant_total:.3f} DT | Reste à payer: {reste_a_payer:.3f} DT"
            ...
            if filename:
                # Export with client info - no columns to exclude since we only show the 3 columns we want
                export_table_to_pdf(self.table, filename, title, client_info)
                QMessageBox.information(self, "Succès", f"Le PDF a été exporté avec succès :\n{filename}")
        else:
            QMessageBox.warning(self, "Erreur", "Impossible de récupérer les informations de la vente.")
...
class App(QWidget):
        def __init__(self):
            ...
            self.btn_update.clicked.connect(lambda: check_updates(self))
            ...
            self.btn_edit.setStyleSheet("""
                ...
                """)
            self.btn_view = QPushButton("👁️ Détails")
            ...
            self.btn_view.clicked.connect(self._handle_view_selected)
            self.btn_edit.clicked.connect(self._handle_edit_selected)
            self.btn_delete.clicked.connect(self._handle_delete_selected)
        ...
        def get_selected_row(self):
            ...
            if row == -1:
                QMessageBox.warning(self, "Attention", "Sélectionnez une ligne d'abord.")
                return None
        ...
        def _handle_view_selected(self):
            client_id = self.get_selected_row()
            if client_id is None:
                return
            self.client_detail(client_id)
        ...
        def _handle_delete_selected(self):
            ...
            self.delete_client(client_id, client_name)
        ...
        def populate_table(self, rows):
            ...
            for r, (cid, nom, tel, adr, eml, credit, reste) in enumerate(rows):
                ...
                name_item = QTableWidgetItem(nom)
...
class UpdateCheckWorker(QObject):
    ...
    def run(self):
        try:
            headers = {"User-Agent": f"ClientFlow/{self.current_version}"}
            r = requests.get(
                "https://api.github.com/repos/YosriSaadi/ClientFlow/releases/latest",
                timeout=10,
                headers=headers,
            )
...
def _pick_asset(assets):
    for asset in assets:
        ...
        name = asset.get("name", "")
        if (url and url.lower().endswith(".exe")) or (name and name.lower().endswith(".exe")):
            return url
    return ""


def _set_update_button_state(self, enabled, text=None):
    btn = getattr(self, "btn_update", None)
    if btn:
...
def _on_update_check_finished(self, release):
    _set_update_button_state(self, True, "🔄 Vérifier les mises à jour")
    try:
        latest_version = str(release.get("tag_name", "")).lstrip("v")
        if not latest_version:
            QMessageBox.warning(self, "Erreur", "Impossible de lire la version de la release.")
            return
        ...
        asset_url = _pick_asset(release.get("assets", []))
        if not asset_url:
            QMessageBox.warning(
                self, "Erreur",
                "Aucun fichier d'installation (.exe) trouvé dans la release."
            )
            return
        ...
        if reply == QMessageBox.Yes:
            download_and_install(self, asset_url)
    except Exception as e:
        QMessageBox.warning(
            self, "Erreur",
...
def _start_update_check(self):
    ...
    _set_update_button_state(self, False, "Vérification...")
    ...
    worker.finished.connect(
        lambda release: _on_update_check_finished(self, release),
        Qt.ConnectionType.QueuedConnection,
    )
    worker.error.connect(
        lambda msg: _on_update_check_error(self, msg),
        Qt.ConnectionType.QueuedConnection,
    )
...
def check_updates(self):
    return _start_update_check(self)
    try:
        r = requests.get(
            "https://api.github.com/repos/YosriSaadi/ClientFlow/releases/latest",
            timeout=5
        ...
        if reply == QMessageBox.Yes:
            download_and_install(self, asset)
...
def download_and_install(self, url):
    try:
        headers = {"User-Agent": f"ClientFlow/{APP_VERSION}"}
        ...
        QMessageBox.information(
            self, "Mise à jour",
            "L’application va se fermer pour lancer la mise à jour."
        )
        ...
        QMessageBox.critical(
            self, "Erreur",
            f"Téléchargement de la mise à jour échoué.\n{str(e)}"
        )
